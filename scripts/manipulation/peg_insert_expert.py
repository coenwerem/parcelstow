"""Scripted expert of the keyed-peg insertion task, pure torch,
importable without the simulator, the parcel_stow_expert.py pattern on
the upright phase schedule.

Acquisition targets come from the bank (assets/peg_insert_bank.json),
...
(assets/peg_insert_trajectory.json). Between knots the expert
...
BANK_PATH = os.path.join(REPO, "assets", "peg_insert_bank.json")
TRAJECTORY_PATH = os.path.join(REPO, "assets", "peg_insert_trajectory.json")

PHASE_NAMES = ["PARK", "APPROACH", "PREGRASP_DWELL", "CLOSE", "GRASP_DWELL", "LIFT", "REORIENT",
               "TRANSFER", "INSERT", "INSERT_DWELL", "RELEASE", "RETREAT", "SETTLE"]
PH = {n: i for i, n in enumerate(PHASE_NAMES)}
DWELL_PHASES = ("PREGRASP_DWELL", "GRASP_DWELL", "INSERT_DWELL", "SETTLE")
KI = 0.08
CORR_CLAMP = 0.35
...
class PegExpert:
    ...
    def __init__(self, actuated_names, bank=None, trajectory=None, device="cpu", candidate=0,
        ...
        self.q_seat_nom = self.phase_knots["INSERT"][1][-1]
    ...
    def target(self, k, f, q_default):
        ...
        arm = torch.where(m, nom + off, arm)
        hand = torch.where(m, self.hand_hold, hand)
        for name in ("TRANSFER", "INSERT", "INSERT_DWELL"):
            m = (k == PH[name]).unsqueeze(1)
            arm = torch.where(m, self._interp(name, f), arm)
            hand = torch.where(m, self.hand_hold, hand)
        m = (k == PH["RELEASE"]).unsqueeze(1)
        arm = torch.where(m, self.q_seat_nom.unsqueeze(0).expand(E, -1), arm)
