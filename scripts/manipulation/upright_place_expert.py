"""Scripted expert of the upright placement task, pure torch,
importable without the simulator, the parcel_stow_expert.py pattern on
the upright phase schedule.

Acquisition targets come from the bank (assets/upright_place_bank.json),
the pregrasp and grasp chain configurations of the planar start-offset
grid entry nearest to the actual object start, with the bank hand
shapes. Manipulation targets come from the IK trajectory
(assets/upright_place_trajectory.json). Between knots the expert
interpolates joint targets linearly in the in-phase fraction. The
per-environment offset between the pose-conditioned lift knot and the
nominal lift knot decays to zero over the REORIENT phase.
...
Phase targets,
  ...
  LOWER           nominal knots, hand grasp
  PLACE_DWELL     place knot, hand grasp (the arm settles before release)
  RELEASE         arm at the place knot, hand cosine blend grasp -> open
  RETREAT         nominal retreat knots, hand open
  SETTLE          arm at the retreat end, hand open

The object is never attached to anything. Physics decides where it
goes.
"""
...
BANK_PATH = os.path.join(REPO, "assets", "upright_place_bank.json")
TRAJECTORY_PATH = os.path.join(REPO, "assets", "upright_place_trajectory.json")

PHASE_NAMES = ["PARK", "APPROACH", "PREGRASP_DWELL", "CLOSE", "GRASP_DWELL", "LIFT", "REORIENT",
               "TRANSFER", "LOWER", "PLACE_DWELL", "RELEASE", "RETREAT", "SETTLE"]
PH = {n: i for i, n in enumerate(PHASE_NAMES)}
DWELL_PHASES = ("PREGRASP_DWELL", "GRASP_DWELL", "PLACE_DWELL", "SETTLE")
KI = 0.08
CORR_CLAMP = 0.35
# The end-shifted grasp carries the object's gravity moment, so after the
# gentle close to the synthesized shape the flexion joints ramp
# CLOSE_OVERDRIVE rad deeper across GRASP_DWELL and hold that squeeze
# through the manipulation; the PD compliance converts the overdrive into
...
class UprightExpert:
    ...
    def __init__(self, actuated_names, bank=None, trajectory=None, device="cpu", candidate=0,
        ...
        self.hand_open = torch.tensor(_named_vector(hand_names, bank["hand_pregrasp"]), **kw)
        overdrive = torch.tensor([CLOSE_OVERDRIVE if n in FLEXION_JOINTS else 0.0 for n in hand_names], **kw)
        self.hand_grasp = torch.tensor(_named_vector(hand_names, bank["hand_grasp"]), **kw)
        ...
        self.grid_pre = torch.tensor([_named_vector(chain, e["q_chain"]) for e in entries], **kw)
        self.grid_grasp = torch.tensor([_named_vector(chain, e["q_chain_grasp"]) for e in entries], **kw)
        self.grid_lift = torch.tensor([_named_vector(chain, e["q_chain_lift"]) for e in entries], **kw)
        self.knots = {}
        for k in traj["knots"]:
        ...
        self.q_place_nom = self.phase_knots["LOWER"][1][-1]
    ...
    def target(self, k, f, q_default):
        ...
        hand = torch.where(m, self.hand_grasp + s * (self.hand_hold - self.hand_grasp), hand)
        ...
        hand = torch.where(m, self.hand_hold, hand)
        m = (k == PH["REORIENT"]).unsqueeze(1)
        nom = self._interp("REORIENT", f)
        off = (1 - s) * (self.q_lift - self.q_lift_nom)
        arm = torch.where(m, nom + off, arm)
        hand = torch.where(m, self.hand_hold, hand)
        for name in ("TRANSFER", "LOWER", "PLACE_DWELL"):
            m = (k == PH[name]).unsqueeze(1)
            arm = torch.where(m, self._interp(name, f), arm)
            hand = torch.where(m, self.hand_hold, hand)
        m = (k == PH["RELEASE"]).unsqueeze(1)
        arm = torch.where(m, self.q_place_nom.unsqueeze(0).expand(E, -1), arm)
        hand = torch.where(m, self.hand_hold + s * (self.hand_open - self.hand_hold), hand)
